...
G0 = 9.80665  # [m/s^2] standard gravity, for rocket-equation use

# ---------------------------------------------------------------------------
# Table 7 reference inertial properties (single "atom")
# ---------------------------------------------------------------------------
M_ATOM = 1250.0                                  # [kg]
I_DIAG = np.array([3571.0, 3571.0, 7118.0])      # [kg m^2] Ixx, Iyy, Izz

# Equivalent thin-disk radius implied by Izz = 0.5 * m * R^2 (the same
# "equal mass distribution" approximation already used in the document to
# derive Table 7). Used here purely as a representative RCS thruster
# moment arm (rim-mounted thrusters), and cross-checked against Ixx.
R_EQ = np.sqrt(2.0 * I_DIAG[2] / M_ATOM)         # [m]
IXX_CHECK = 0.25 * M_ATOM * R_EQ**2

ALT_GRID = np.array([500, 600, 700, 800, 900, 1000], dtype=float)
H_REF = 600.0  # primary design altitude (target orbit, per Section 2)

# Area-to-mass / disturbance-torque assumptions (explicitly stated; not
# derivable from the document, which gives no CoM/CoP offset)
AM_NOMINAL = 0.0225          # [m^2/kg], median of the 0.005-0.04 grid used throughout
CP_CM_OFFSET_M = 0.10        # [m] assumed worst-case aero/SRP center-of-pressure
                              # offset from the center of mass (asymmetric
                              # deployables/antennas); stated assumption, not derived


# ---------------------------------------------------------------------------
# 1) Disturbance torques: gravity-gradient, aerodynamic, SRP
# ---------------------------------------------------------------------------
def gravity_gradient_torque_max(h_km, I_diag=I_DIAG):
    """
    Max gravity-gradient torque [N*m] about the intermediate axis, worst-case
    45 deg tilt between the principal axis of largest inertia and the local
    vertical:
        T_gg_max = 1.5 * n^2 * |I3 - I1|
    with n the orbital mean motion. (Standard result, e.g. Wertz, SMAD.)
    """
    a = op.R_EARTH + h_km
    n2 = op.MU_EARTH / a**3       # = n^2 [rad^2/s^2]
    return 1.5 * n2 * (I_diag[2] - I_diag[0])


def aero_torque(h_km, am=AM_NOMINAL, offset_m=CP_CM_OFFSET_M, regime="nominal"):
    """Order-of-magnitude aerodynamic disturbance torque [N*m]: F_drag * offset."""
    if regime == "fast":
        a_drag = op.drag_accel(h_km, 0.04, 0.0, Cd=2.2)     # rho_max, high A/m
    elif regime == "slow":
        a_drag = op.drag_accel(h_km, 0.005, 180.0, Cd=2.2)  # rho_min, low A/m
    else:
        _, a_drag, _ = op.drag_accel_envelope(h_km, np.linspace(0.005, 0.04, 12))
    F_drag = a_drag * M_ATOM
    return F_drag * offset_m


def srp_torque(am=AM_NOMINAL, offset_m=CP_CM_OFFSET_M):
    """Order-of-magnitude SRP disturbance torque [N*m]."""
    a_srp = op.srp_accel(am, Cr=1.3)
    F_srp = a_srp * M_ATOM
    return F_srp * offset_m


def fig_disturbance_torques():
    h = np.linspace(500, 1000, 60)
    Tgg = np.array([gravity_gradient_torque_max(hh) for hh in h])
    Taero_nom = np.array([aero_torque(hh, regime="nominal") for hh in h])
    Taero_fast = np.array([aero_torque(hh, regime="fast") for hh in h])
    Tsrp = np.full_like(h, srp_torque())

    fig, ax = plt.subplots()
    ax.plot(h, Tgg * 1e3, label="Gravity-gradient (worst-case 45$^\\circ$ tilt)", color="#1f4e79")
    ax.plot(h, Taero_nom * 1e3, label="Aerodynamic (nominal drag, 0.10 m CoP-CoM offset)",
            color="#3182bd", linestyle="--")
    ax.plot(h, Taero_fast * 1e3, label="Aerodynamic (fast/high-density drag case)",
            color="#08306b", linestyle=":")
    ax.plot(h, Tsrp * 1e3, label="SRP (0.10 m CoP-CoM offset)", color="#b8860b", linestyle="-.")
    ax.set_yscale("log")
    ax.set_xlabel("Altitude [km]")
    ax.set_ylabel("Disturbance torque [mN$\\cdot$m]")
    ax.set_title("Single-Atom Disturbance Torque Budget vs. Altitude\n"
                 "(Table 7 inertia; CoP-CoM offset assumption, see text)")
    ax.legend(fontsize=9.5, loc="upper right")
    fig.savefig(os.path.join(OUT_DIR, "fig9_disturbance_torques.png"))
    plt.close(fig)
    i600 = np.argmin(np.abs(h - 600))
    return {"Tgg_600": Tgg[i600], "Taero_nom_600": Taero_nom[i600],
            "Taero_fast_600": Taero_fast[i600], "Tsrp_600": Tsrp[i600]}
